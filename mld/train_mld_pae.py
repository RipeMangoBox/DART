...
class Trainer:
    def __init__(self, args):
        ...
        phase_num = 5 if denoiser_args.pae_type == 'phase' else 4
        denoiser_model_args.noise_shape = (mvae_args.model_args.latent_dim[0], mvae_args.model_args.latent_dim[1] * phase_num) # sx, sy, f, a, b
        denoiser_class = DenoiserMLP if isinstance(denoiser_model_args, DenoiserMLPArgs) else DenoiserTransformer
        print('(#^.^#) denoiser class:', denoiser_class)
        ...
        if not os.path.exists(statistics_path):
            ...
            for primitive_idx in range(num_primitive):
                with amp.autocast(enabled=True, dtype=torch.float16):
                    ...
                    p, f, a, b = params
                    latent_parameterization.append(torch.cat([f, a, b], dim=1))
    ...
    def calc_loss(self, motion, cond, history_motion, future_motion_gt, future_motion_pred, manifold_gt, manifold_pred, weights):
        ...
        # latent rec loss
        latent_rec_loss = self.rec_criterion(manifold_pred, manifold_gt)
