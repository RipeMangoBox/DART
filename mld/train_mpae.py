...
class Trainer:
    ...
    def calc_loss(self, motion, cond, history_motion, future_motion_gt, future_motion_pred):
        ...
        loss = train_args.weight_rec * rec_loss + \
    ...
    def get_latent_scale(self, model):
        ...
        with torch.no_grad():
            ...
            params, latent = model.encode(future_motion=future_motion_gt, history_motion=history_motion)  # [1, B, D]
        ...
        with torch.no_grad():
            ...
            for _ in tqdm(range(max(128, len(self.val_dataset) // self.batch_size))):
                ...
                for primitive_idx in range(num_primitive):
                    ...
                    params, _ = model.encode(future_motion=future_motion_gt, history_motion=history_motion)
                    future_motion_pred = model.decode(params, history_motion, nfuture=future_length)
