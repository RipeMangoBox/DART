...
class Trainer:
    ...
    def get_latent_scale(self, model):
        ...
        with torch.no_grad():
            ...
            all_mean = future_motion_pred.mean()
            all_std = (future_motion_pred - all_mean).pow(2).mean().sqrt()
    ...
    def validate(self):
        ...
        for k, v in losses_dict.items():
            losses_dict[k] = [x.to(torch.float) for x in v] if isinstance(v, list) else [torch.tensor(v, device=self.device)]
