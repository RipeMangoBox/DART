...
class AutoMldVae(nn.Module):
    def __init__(self,
        ...
        self.global_motion_token = nn.Parameter(
            torch.randn(self.latent_size, self.h_dim))
    ...
    def decode(self, z: Tensor, history_motion, nfuture,
        ...
        if scale_latent:  # only used during denoiser training
            z = z * self.latent_std # [bs, nframes, latent_dim]
    ...
    def forward(self, future_motion, history_motion, nfuture):
        ...
        xseq = self.query_pos_encoder(xseq)
        dist = self.encoder(xseq)[:dist.shape[0]]  # [1 or 2*latent_size, bs, h_dim]
        dist = self.encoder_latent_proj(dist)  # [1 or 2*latent_size, bs, latent_dim]
        
        if self.rep_mode == "bottleneck":
            z, commit_loss, metrics = self.bottleneck.forward(dist) # (1, bs, latent_dim)
        elif self.rep_mode == "vae":
            # Switch sequence and batch_size because the input of
            # Pytorch Transformer is [Sequence, Batch size, ...]
        ...
        output = self.final_layer(output)
        # Pytorch Transformer: [Sequence, Batch size, ...]
        motion_recon = output.permute(1, 0, 2)  # [nfuture, bs, nfeats] -> [bs, nfuture, latent_dim]
...
class AutoMldPae(nn.Module):
    ...
    def decode(self, params: List[Tensor], history_motion, nfuture, scale_latent: bool = False):
        ...
        """ 接收 PAE 参数进行正弦重建 """
        p, f, a, b = params # [bs, latent_dim, 1]
...
class AutoMldPae_Phase(AutoMldPae):
    ...
    def decode_from_manifold(self, manifold, history_motion, nfuture, scale_latent: bool = False):
        '''
        manifold: [bs, 5 * latent_dim, T=1]
        '''
        assert torch.isnan(manifold).any() == False

        sx, sy, f, a, b = manifold.split(self.latent_dim, dim=1)    # 3 * [bs, latent_dim, T=1]
        ...
        normed_latent_parameterization = torch.cat([f, a, b], dim=1) # [bs, 3 * latent_dim, T=1]
        latent_parameterization = normed_latent_parameterization * self.latent_param_std + self.latent_param_mean
        f, a, b = latent_parameterization.split(self.latent_dim, dim=1) # [bs, latent_dim, T=1]
...
class AutoMldPae_P(AutoMldPae):
    ...
    def decode_from_manifold(self, manifold: List[Tensor], history_motion, nfuture, scale_latent: bool = False):
        '''
        manifold: [bs, 4 * latent_dim, T=1]
        '''
        p, f, a, b = manifold.split(self.latent_dim, dim=1) # 4 * [bs, latent_dim, T=1]
        normed_latent_parameterization = torch.cat([f, a, b], dim=1) # [bs, 3 * latent_dim, T=1]
        latent_parameterization = normed_latent_parameterization * self.latent_param_std + self.latent_param_mean
        f, a, b = latent_parameterization.split(self.latent_dim, dim=1) # [bs, latent_dim, T=1]
        params = [p, f, a, b]
