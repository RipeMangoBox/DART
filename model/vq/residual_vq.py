...
class ResidualVQ(nn.Module):
    ...
    def encode(self, x, encode_mode="quantizeds"):
        quantized_out = 0.
        residual = x
        ...
        with autocast(enabled = False):
            for quantizer_index, layer in enumerate(self.layers):
                # quantized, *rest = layer(residual, indices = layer_indices) #single quantizer TODO
                ...
                all_quantizeds.append(quantized)

        return quantized_out if encode_mode == "quantizeds" else torch.stack(all_quantizeds, dim=-2)

    def decode(self, x, decode_mode="quantizeds"):
        if decode_mode == "quantizeds":
            output = x
        elif decode_mode == "all_quantizeds":
            if len(x.shape) == 3:
                b, n, d = x.shape
                x = x.view(b, n, 2, d//2)   # num_quantizer = 2,  hard coding for now
    ...
    def forward(self, x, return_all_codes = False, sample_codebook_temp = None, force_dropout_index=-1):
        ...
        all_perplexity = {'perplexity': sum(all_perplexity)/len(all_perplexity)}

        # quantized, all_indices, commit_loss, all_metrics
