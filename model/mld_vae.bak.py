...
class AutoMldPae(nn.Module):
    def __init__(self,
                 nfeats: int,
                 latent_dim: tuple = [1, 256], # PAE模式下，latent_dim[-1] 应与 PAE 的 embedding_channels 对应
                 h_dim: int = 512,
        ...
        self.embedding_channels = latent_dim[-1] # PAE 的分量数
        self.latent_size = latent_dim[0]
        ...
        if pae_combine_mode == "conv":
            # 使用方案一提到的卷积提取器，实现语义浓缩并保持 time_range
        ...
        self.to_phase_latent = nn.Linear(h_dim, self.embedding_channels)

        # PAE 关键：相位投影 (对应 PAE 的 self.fc)
        self.phase_fc = nn.ModuleList([nn.Linear(time_range, 2) for _ in range(self.embedding_channels)])

        # --- 3. 后处理结构 ---
        self.final_layer = nn.Linear(self.embedding_channels, nfeats)
        
        self.register_buffer('latent_mean', torch.tensor(0))
        self.register_buffer('latent_std', torch.tensor(1))

    def FFT(self, y, dim=2):
        """ 这里的 y 形状为 [bs, embedding_channels, time_range] """
        rfft = torch.fft.rfft(y, dim=dim)
        magnitudes = rfft.abs()
    ...
    def encode(self, future_motion, history_motion, scale_latent: bool = False):
        ...
        # 3. 映射到 PAE 隐空间 [bs, seq, h_dim] -> [bs, embedding_channels, seq]
        ...
        # 5. 计算相位 (Atan2)
        p = torch.empty((bs, self.embedding_channels), device=y.device)
        for i in range(self.embedding_channels):
            v = self.phase_fc[i](y[:, i, :]) # 输入整个时间窗口
            p[:, i] = torch.atan2(v[:, 1], v[:, 0]) / self.tpi

        #Parameters    
        # p = p.unsqueeze(2)  # (bs, embedding_channels, 1)
        # f = f.unsqueeze(2)  # (bs, embedding_channels, 1)
        # a = a.unsqueeze(2)  # (bs, embedding_channels, 1)
        # b = b.unsqueeze(2)  # (bs, embedding_channels, 1)
        # # params = [p, f, a, b]
        params = torch.cat([p, f, a, b], dim=1).unsqueeze(0) # (1, bs,  4 *embedding_channels)
    ...
    def decode(self, params, nfuture):
        """ 接收 PAE 参数进行正弦重建 """
        p, f, a, b = params.permute(1, 2, 0).split(self.embedding_channels, dim=1) # 4 * (bs, embedding_channels, 1)

        # 1. 信号重建 (PAE 核心公式)
        # y: [bs, embedding_channels, time_range]
