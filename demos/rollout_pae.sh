...
model_list=(
'./mld_denoiser/mld_pae_babel_smplx/checkpoint_300000.pt'
)

for model in "${model_list[@]}"; do
  python -m mld.rollout_mld_pae --denoiser_checkpoint "$model" --batch_size $batch_size  --text_prompt "$text_prompt" --guidance_param $guidance --respacing "$respacing" --export_smpl $export_smpl --zero_noise $zero_noise --use_predicted_joints $use_predicted_joints  --dataset "$dataset"  --fix_floor $fix_floor
done
