...
class WeightedPrimitiveSequenceDataset:
    def __init__(self, dataset_name='weighted_mp_seq',
        ...
        self.embedding_path = embedding_path = Path(dataset_path, f'{split}_music_dict.pkl')
        if embedding_path.exists():
            print(f'loading text embeddings from {embedding_path}')
            with open(embedding_path, 'rb') as f:
                self.music_dict = pickle.load(f)
        else:
            ...
            while batch_start_idx < num_texts:
                ...
                musics.append(encode_text(self.clip_model, raw_texts[batch_start_idx:batch_end_idx]))
                batch_start_idx = batch_end_idx
            musics = torch.cat(musics, dim=0).detach().cpu().numpy()
            print(musics.shape)
            self.music_dict = {raw_texts[idx]: musics[idx] for idx in range(num_texts)}
            self.music_dict[''] = np.zeros(512).astype(np.float32)  # for empty text have zero embedding, compatible with mdm text masking
            with open(embedding_path, 'wb') as f:
                pickle.dump(self.music_dict, f)
        for key in self.music_dict:
            self.music_dict[key] = torch.from_numpy(self.music_dict[key]).to(dtype=torch.float32, device=self.device)

    def update_music_dict(self, new_texts):
        new_musics = encode_text(self.clip_model, new_texts)
        for idx, text in enumerate(new_texts):
            self.music_dict[text] = new_musics[idx]

    def export_music_dict(self):
        music_dict = {key: self.music_dict[key].detach().cpu().numpy() for key in self.music_dict}
        with open(self.embedding_path, 'wb') as f:
            pickle.dump(music_dict, f)
    ...
    def get_batch(self, batch_size=8):
        ...
        for gender in ['female', 'male']:
            ...
            for primitive_idx in range(self.num_primitive):
                ...
                unseen_texts = [text for text in primitive_texts if text not in self.music_dict]
                if len(unseen_texts) > 0:
                    self.update_music_dict(unseen_texts)
                music = torch.stack([self.music_dict[text] for text in primitive_texts], dim=0)  # [B, 512]
                gender_batch.append(
                    {
                        'texts': primitive_texts,
            ...
            #     history_motion = torch.zeros_like(motion_tensor_normalized, dtype=torch.float32, device=self.device)
            #     history_motion[..., :self.cfg.history_length] = motion_tensor_normalized[..., :self.cfg.history_length]
            #     music = torch.stack([self.music_dict[text] for text in primitive_texts],
            #                                  dim=0)  # [B, 512]
            #
            #     gender_batch.append(
            #         {
...
class WeightedPrimitiveSequenceDatasetV2(WeightedPrimitiveSequenceDataset):
    def __init__(self, dataset_name='weighted_mp_seq',
        ...
        self.enforce_gender = enforce_gender
        self.enforce_zero_beta = enforce_zero_beta
        # self.text_tolerance = text_tolerance # Removed
        print('enforce_gender: ', enforce_gender)
        print('enforce_zero_beta: ', enforce_zero_beta)
        ...
        if load_data:
            ...
            for data in raw_dataset:
                # Support both nested 'motion' dict (old) and flat dict (new)
                trans_len = len(data['motion']['trans']) if 'motion' in data else len(data['trans'])
                if trans_len >= self.seq_length:
                    dataset.append(data)
            
            for data in dataset:
                # Standardize data structure to nested 'motion' dict
                if 'motion' not in data:
                    # Map flat structure (from new data process) to class expected structure
                    motion_data = {
                        'gender': data['gender'],
                        'betas': data['betas'],
                        'trans': data['trans'],
            ...
            # --- Weight Calculation (Simplified for Music/Uniform) ---
            # Note: Removed 'text' specific weighting logic as text is replaced by music
            # Keeping 'uniform' and 'length' as primary schemes
    ...
    def calc_mean_std(self, batch_size=512):
        # ... (Same as original, primitive_utility handles motion feature normalization) ...
        ...
        for seq_data in self.dataset:
            ...
            while batch_start_idx < len(primitive_dict['transl']):
                ...
                # Trim the last frame as it was used for velocity calc
    ...
    def get_primitive(self, seq_data, start_frame, end_frame, skip_text=True):
        ...
        # Motion data: extract T+1 frames for velocity calculation
        primitive_dict = {
            'gender': motion_data['gender'],
            'betas': motion_data['betas'].expand(1, self.primitive_length + 1, 10),
        ...
        output = {
            'text': '', # Keep empty text for compatibility
            'primitive_dict': primitive_dict,
        }
        return output
    ...
    def get_batch(self, batch_size=8):
        ...
        for seq_idx in batch_idx:
            ...
            # Random Start Frame
            start_frame = random.randint(0, num_frames - self.seq_length) 
